...
// [[Rcpp::export]]
List cpc_to_hs_cpp(const std::vector<std::string> &hs_codes,
                   const std::vector<std::string> &cpc_codes,
                   const std::vector<std::string> &codes)
{
    // Define objects
    const int n = hs_codes.size();
    const int m = codes.size();
    List results(m);

    std::unordered_multimap<std::string, std::string> cpc_hs_conversion_table(n);
    ...
    for (int i = 0; i < n; i++)
    {
        cpc_hs_conversion_table.insert({cpc_codes[i], hs_codes[i]});
    }
    ...
    for (int j = 0; j < m; j++)
    {
        auto range = cpc_hs_conversion_table.equal_range(codes[j]);
